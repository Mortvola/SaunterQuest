...
class Route {
  constructor(hikeId, map) {
    ...
    this.elevationMarker = new TrailMarker(elevationUrl);
  ...
  newWaypoint(anchor) {
    ...
    const waypoint = new TrailMarker(wayPointUrl);
  ...
  static addPointsToArray(anchor, route) {
    ...
    if (anchor.trail && anchor.trail.length > 0) {
      if (anchor.lat === anchor.trail[0].lat && anchor.lng === anchor.trail[0].lng) {
