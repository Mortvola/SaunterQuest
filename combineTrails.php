	...
	while (count($trails) > 0)
	{
		...
		for ($t = 0; $t < count($trails);)
		{
			$d1 = haversineGreatCircleDistance(
					$trails[$t]->route[0]->lat, $trails[$t]->route[0]->lng,
					$combinedTrail->route[count($combinedTrail->route) - 1]->lat, $combinedTrail->route[count($combinedTrail->route) - 1]->lng);
			
			$d2 = haversineGreatCircleDistance(
					$trails[$t]->route[count($trails[$t]->route) - 1]->lat, $trails[$t]->route[count($trails[$t]->route) - 1]->lng,
					$combinedTrail->route[0]->lat, $combinedTrail->route[0]->lng);
