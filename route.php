...
function deletePoints(&$segments, $routeUpdate)
{
    ...
    if ($routeUpdate->index > 0 && $routeUpdate->index < count($segments)
        ...
        $result = getTrail(
            $prevSegment->lat,
            $prevSegment->lng,
            $prevSegment->trailName,
            $prevSegment->trailIndex,
...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    $fileName = getRouteFileName($route->userHikeId);
    ...
    for ($index = 0; $index < count($route->anchors); $index++) {
        ...
        if ($index < count($route->anchors) - 1) {
            $distance += haversineGreatCircleDistance(
                $route->anchors[$index]->lat,
                $route->anchors[$index]->lng,
                $route->anchors[$index + 1]->lat,
                $route->anchors[$index + 1]->lng
            );
        }
    }
