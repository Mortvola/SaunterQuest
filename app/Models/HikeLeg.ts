...
export default class HikeLeg extends BaseModel {
  ...
  public async getFullRoute(this: HikeLeg): Promise<RoutePoint[]> {
  ...
  public async addEndpoint(
    ...
    if (this.routePoints.length === 0) {
      return this.prepareUpdates(await this.setStart(routeGroupId, point), 'start');
    }
  ...
  private validateUpdate(this: HikeLeg) {
  ...
  private async findRouteBetweenWaypoints(
    this: HikeLeg,
    routeGroupId: number | null,
    ...
    const newAnchorsArray = await HikeLeg.findPath(routeGroupId, anchors);
    ...
    if (newAnchorsArray) {
      for (let a = 0; a < anchors.length - 1; a += 1) {
        ...
        const anchor1 = Object.assign(
          ...
          HikeLeg.transferRoutePointInfo(newAnchors[0]),
        );
        ...
        for (let i = 1; i < newAnchors.length - 1; i += 1) {
          const routePoint = Object.assign(
            new RoutePoint(),
            HikeLeg.transferRoutePointInfo(newAnchors[i]),
          );
        ...
        const anchor2 = Object.assign(
          ...
          HikeLeg.transferRoutePointInfo(newAnchors[newAnchors.length - 1]),
        );
  ...
  private static async findPath(routeGroupId: number | null, anchors: RoutePoint[]): Promise<Path> {
    ...
    const options = encodeURIComponent(JSON.stringify({
      preferredTrailId: routeGroupId !== -1 ? routeGroupId : undefined,
    }));
  ...
  public async setStart(
    ...
    if (anchor === undefined) {
      ...
      if (nextAnchorIndex !== null) {
        ...
        await this.findRouteBetweenWaypoints(routeGroupId, [0, nextAnchorIndex]);
  ...
  private async updateSortOrder(this: HikeLeg) {
  ...
  public async updateSchedule(this: HikeLeg, user: User): Promise<void> {
  ...
  public async updateWaypointPosition(
    ...
    if (
      ...
      && HikeLeg.edgeFractionBetweenAnchors(
        trailInfo.line_id,
        trailInfo.fraction,
    ...
    if (prevWaypointIndex === null) {
      ...
      await this.findRouteBetweenWaypoints(routeGroupId, [waypointIndex, nextWaypointIndex]);
    ...
    if (nextWaypointIndex === null) {
      ...
      await this.findRouteBetweenWaypoints(routeGroupId, [prevWaypointIndex, waypointIndex]);
    ...
    await this.findRouteBetweenWaypoints(
      routeGroupId, [prevWaypointIndex, waypointIndex, nextWaypointIndex],
    );
  ...
  public async setEnd(
    ...
    if (anchor === undefined || this.routePoints.length === 1) {
      ...
      if (prevAnchorIndex !== null) {
        ...
        await this.findRouteBetweenWaypoints(
          routeGroupId, [prevAnchorIndex, this.routePoints.length - 1],
        );
  ...
  public async addWaypoint(
    ...
    if (this.routePoints.length === 1) {
      return this.prepareUpdates(await this.setEnd(routeGroupId, point), 'end');
    }
    ...
    if (trailInfo !== null) {
      this.routePoints
        .some((_, index) => {
          if (index < this.routePoints.length - 1) {
            if (HikeLeg.edgeFractionBetweenAnchors(
              trailInfo.line_id,
              trailInfo.fraction,
  ...
  private findNearestWaypoint(this: HikeLeg, point: Point) {
    ...
    for (let i = 0; i < this.routePoints.length - 1; i += 1) {
      ...
      const [s, distance] = HikeLeg.nearestSegmentFind(point.lat, point.lng, anchor.trail);
  ...
  public async deleteWaypoint(
    ...
    await this.findRouteBetweenWaypoints(routeGroupId, [prevAnchorIndex, nextAnchorIndex]);
